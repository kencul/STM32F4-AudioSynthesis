...
extern "C" void cpp_main() {
  ...
  while (1) {
    ...
    if (currentTick - lastPrint > 500) {
      lastPrint = currentTick;
      printf("elapsed_us=%lu\n", elapsed_us);
    }
    ...
    if (currentView != VIEW_WAVETABLE &&
        (currentTick - lastInteractionTime > UI_TIMEOUT_MS)) {
...
extern "C" void HAL_I2S_TxCpltCallback(I2S_HandleTypeDef *hi2s) {
  ...
  std::fill(buffer + Constants::BUFFER_SIZE,
            buffer + (Constants::CIRCULAR_BUFFER_SIZE), 0);
  voiceManager.process(&buffer[Constants::BUFFER_SIZE]);
...
void updateOledView() {
  ...
  auto formatTime = [](char *buf, size_t len, const char *label, float sec) {
    if (sec < 1.0f)
      snprintf(buf, len, "%s: %.0fms", label, sec * 1000.0f);
    else
      snprintf(buf, len, "%s: %.2fs", label, sec);
  };
  ...
  case VIEW_WAVETABLE: {
    ...
    for (int i = 0; i <= barWidth; ++i)
      oled.drawPixel(barStart + i, barY, true);
  ...
  case VIEW_FILTER:
    ...
    if (lastChangedIndex == 2)
      snprintf(msg, sizeof(msg), "RESONANCE: %.2f", params.resonance);
    else if (params.cutoff < 1000.0f)
      snprintf(msg, sizeof(msg), "CUTOFF: %.0fHz", params.cutoff);
    else
      snprintf(msg, sizeof(msg), "CUTOFF: %.1fkHz", params.cutoff / 1000.0f);
    break;

  case VIEW_ADSR:
    AdsrVisualizer::draw(oled, params.attack, params.decay, params.sustain,
                         params.release);
    if (lastChangedIndex == 4)
      formatTime(msg, sizeof(msg), "ATTACK", params.attack);
    else if (lastChangedIndex == 5)
      formatTime(msg, sizeof(msg), "DECAY", params.decay);
    else if (lastChangedIndex == 6)
      snprintf(msg, sizeof(msg), "SUSTAIN: %.2f", params.sustain);
    else if (lastChangedIndex == 7)
      formatTime(msg, sizeof(msg), "RELEASE", params.release);
    else
      snprintf(msg, sizeof(msg), "ENV GENERATOR");
    break;
  }
  ...
  if (lastChangedIndex == 0 && (now - lastInteractionTime < UI_TIMEOUT_MS)) {
    for (int i = 0; i < 7; ++i)
      oled.drawLine(0, i, 127, i, false); // Clear header
...
void playStartupSequence() {
  ...
  for (float angleOffset = 0; angleOffset < finalPhaseEnd;
       angleOffset += 0.05f) {
    while (oled.isBusy())
      ;
    oled.fill(false);
    ...
    for (uint8_t i = 0; i < 8; i++)
      ledController.setChannel(i, breathe);
    ...
    for (int i = 0; i <= 6; i++) {
      ...
      if (i == 0) {
        ...
        oled.drawLine(lastX, lastY, x, y, true);
      }
      lastX = x;
      lastY = y;
    }
